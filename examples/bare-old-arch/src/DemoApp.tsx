...
export default function DemoApp() {
  // Defer native reads until after mount — on Old Arch iOS, sync native access during the
  // first render can deadlock the JS thread (white screen). Defaults match a disabled build.
  const [enabled, setEnabled] = useState(false);
  const [debugOnly, setDebugOnly] = useState(true);

  useEffect(() => {
    setEnabled(isFlipperEnabled());
    setDebugOnly(isFlipperDebugOnly());
  }, []);
  ...
  return (
    <View style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container} testID='flipper-example-scroll'>
          ...
          <Text style={styles.subtitle}>bare-old-arch · RN 0.76 · Old Architecture</Text>
          {Platform.OS !== 'android' && (
